...
<h2 class="page-header">
    <a class="js-creat-file pull-right btn btn-danger ml10" style="margin-left:10px;" type="button" href="<?php echo U('Task/flow');?>"><span class="fa fa-stack-overflow"></span> 任务流管理</a>
    <a href="<?php echo U('Task/edit');?>" type="button" class="pull-right btn btn-success"><span class="glyphicon glyphicon-plus"></span> 新增任务</a>
            任务管理
</h2>
<form id="form" method="post" action="<?php echo U('Upload/index')?>">
    <table border="1" cellspacing="0" cellpadding="10" class="table table-bordered">
        <thead>
        <tr>
            <td class="w80 tc">任务编号</td>
            <td>任务名称</td>
            <td>任务类型</td>
            <td>任务状态</td>
        ...
        <tbody>
        <?php if(!empty($task_list)): if(is_array($task_list)): $i = 0; $__LIST__ = $task_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                    <td><?php echo ($vo['id']); ?></td>
                    <td><?php echo ($vo['name']); ?></td>
                    <td>
                        <?php switch($vo["type"]): case "1": ?>普通<?php break;?>
                            <?php case "2": ?>日常<?php break; endswitch;?>
                    </td>
                    <td>
                         <?php switch($vo["is_del"]): case "0": ?>开启<?php break;?>
                            <?php case "1": ?>关闭<?php break; endswitch;?>
                    </td>
                    <td><a href="<?php echo U('edit',array('guid'=>$vo['guid']));?>"><i class="glyphicon glyphicon-search"></i></a></td>
