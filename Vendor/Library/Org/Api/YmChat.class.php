...
class YmChat {
     ...
     * @param string $type txt, img, audio...
     ...
     * @return string
	 */
	function sendMsg($from_user = "admin", $username, $type, $content, $target_type = "users", $ext = array()) {
		$option ['target_type'] = $target_type;
		$option ['target'] = $username;
        $option ['msg'] = array('type'=>$type, 'msg'=>$content);
	 ...
	 *        	$group_id
	 * @param
	 *        	$usernames 添加多个 array
	 */
	public function addGroupsUsers($group_id, $usernames) {
        $options = array('usernames'=>$usernames);
		$url = $this->url . "chatgroups/" . $group_id . "/users";
		$access_token = $this->getToken ();
		$header [] = 'Authorization: Bearer ' . $access_token;
	...
	/**
	 * 批量删除群组成员
	 * @param string $group_id 群组chat_id
	 * @param array $usernames 用户名数组
	 * @return int
	 */
	public function delGroupsUsers($group_id, $usernames) {
	...
	public function getToken() {
		...
		$url = $this->url . "token";
//        $file_path = RUNTIME_PATH.'/Data/api/token.txt';
