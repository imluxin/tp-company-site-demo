...
abstract class Driver {
            ...
            if(is_string($val[0])) {
                ...
                if(preg_match('/^(eq|neq|gt|egt|lt|elt)$/',$exp)) { // 比较运算
                    ...
                    if(isset($val[2]) && 'exp'==$val[2]) {
                        ...
                        // 修改 by ylx
                        if(empty($val[1])) {
                            $zone = "'yunmai365-placeholder-".md5(uniqid())."'";
                        } else {
                            $zone = implode(',', $this->parseValue($val[1]));
                        }
                        // 修改结束
   ...
   /**
     ...
     * @return false | integer
     */
    public function inserUp($data,$options=array()) {
        $values  =  $fields  = $updates = array();
        $this->model  =   $options['model'];
        foreach ($data as $key=>$val){
        ...
        $sql   =  'INSERT INTO '.$this->parseTable($options['table']).' ('.implode(',', $fields).') VALUES ('.implode(',', $values).') ON DUPLICATE KEY UPDATE '.implode(',', $updates).';';
     ...
     * @return false | integer
     */
    public function inserUpAll($dataSet,$options=array(),$replace=false) {
        $values  =  array();
        $this->model  =   $options['model'];
        ...
        $sql   =  'INSERT INTO '.$this->parseTable($options['table']).' ('.implode(',', $fields).') '.implode(' UNION ALL ',$values) .' ON DUPLICATE KEY UPDATE '.implode(',', $updates).';';
        $sql   .= $this->parseComment(!empty($options['comment'])?$options['comment']:'');
        return $this->execute($sql,!empty($options['fetch_sql']) ? true : false);
    }
}